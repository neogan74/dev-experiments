...
## Python faker alternative

Prefer Python? Install the helpers and run the threaded loader.

```bash
python3 -m venv .venv && source .venv/bin/activate
pip install -r requirements.txt

# Load 50k keys with 32 threads
python scripts/etcd_faker.py --count 50000 --concurrency 32 --prefix=datasets/python-run

# Limit value types or hit multiple endpoints
python scripts/etcd_faker.py --types text,json --endpoints=localhost:2379,localhost:22379
```

Flags mirror the Go tool (count, concurrency, prefix, value types, binary size, seed, credentials). Progress logs include aggregate throughput so you can compare language implementations.

### Python reporter

```bash
# Summarize a prefix with JSON price stats
python scripts/etcd_reporter.py --prefix=datasets/python-run

# Page in batches or cap the scan
python scripts/etcd_reporter.py --batch-size 5000 --limit 20000 --serializable
```

Outputs list key counts per value type, human-readable storage totals, and average/min/max prices for JSON payloads with a `price` field. Useful for quick validation after large Python or Go faker runs.
